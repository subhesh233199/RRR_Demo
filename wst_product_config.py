...
table_indices = {
    "release_scope": (1, 2),         # (page_number, table_index)
...
def setup_crew_wst(extracted_text: str, versions: list, llm=llm):
    ...
    DATA_CREW_PROMPT = f"""
Given the following combined text for multiple versions of a WST release readiness report, each with three tables ("Release Scope", "Critical Metrics", "Health Trends") and version labels:

Your tasks:
- For each version, extract data for all three tables and structure it as JSON.
- For each table, include only the metrics and columns specified in the product config.
- The order of tables, metrics, and columns MUST match the order provided below.
- Missing values should be "N/A".
- Output a single JSON object with top-level keys: "release_scope", "critical_metrics", "health_trends", each mapping to a dict with version numbers as keys and lists of dicts as values (each dict = a row/metric).
- Strictly preserve numeric types where possible, else return as strings.

Release Scope Table:
  Metrics: {', '.join(RELEASE_SCOPE_METRICS)}
  Columns: {', '.join(RELEASE_SCOPE_COLUMNS)}

Critical Metrics Table:
  Metrics: {', '.join(CRITICAL_METRICS)}
  Columns: {', '.join(CRITICAL_COLUMNS)}

Health Trends Table:
  Metrics: {', '.join(HEALTH_TRENDS_METRICS)}
  Columns: {', '.join(HEALTH_TRENDS_COLUMNS)}

EXAMPLE OUTPUT:
{{
  "release_scope": {{"45.1.15.0": [...], "45.1.16.0": [...] }},
  "critical_metrics": {{...}},
  "health_trends": {{...}}
}}
...
def run_fallback_visualization(metrics):
    """
    Fallback: generates minimal line charts for all key metrics from the metrics JSON.
    Saves each chart as a .png in the 'visualizations' folder.
    Covers Release Scope (ATLs Fixed), Critical Metrics (ATLs from Metrics col), Health Trends (Current Release Metrics).
    """
    ...
    # 1. Release Scope: ATLs Fixed trend for each metric
    for metric in metrics.get("release_scope", {}):
        versions = []
        atls_values = []
        for version, rows in metrics["release_scope"].items():
            for row in rows:
                # Each row is a dict keyed by column name
                if row.get("ATLs Fixed") not in (None, "N/A", ""):
                    try:
                        atls = float(row["ATLs Fixed"])
                        versions.append(version)
                        atls_values.append(atls)
                    except Exception:
                        continue
        if versions and atls_values:
            plt.figure()
            plt.plot(versions, atls_values, marker='o')
            plt.title(f"ATLs Fixed Trend - {metric}")
            plt.xlabel('Version')
            plt.ylabel('ATLs Fixed')
            plt.tight_layout()
            plt.savefig(os.path.join(viz_folder, f"{metric}_atls_fixed.png"))
            plt.close()

    # 2. Critical Metrics: ATLs from "Metrics (WST {Version})"
    for metric in metrics.get("critical_metrics", {}):
        versions = []
        atls_values = []
        for version, rows in metrics["critical_metrics"].items():
            for row in rows:
                # Parse ATLs value from Metrics (WST {Version}) column, e.g., "ATLs: 177 issues (1 issue Open)"
                mval = row.get(f"Metrics (WST {version})", "") or row.get("Metrics", "")
                atls = None
                if isinstance(mval, str) and "ATLs:" in mval:
                    try:
                        # crude extraction: "ATLs: 177 issues" or similar
                        atls_str = mval.split("ATLs:")[1].split("issues")[0]
                        atls = float("".join([c for c in atls_str if c.isdigit() or c == "."]))
                    except Exception:
                        continue
                if atls is not None:
                    versions.append(version)
        ...
        if versions and atls_values:
            plt.figure()
            plt.plot(versions, atls_values, marker='o')
            plt.title(f"ATLs Trend - {metric}")
            plt.xlabel('Version')
            plt.ylabel('ATLs')
            plt.tight_layout()
            plt.savefig(os.path.join(viz_folder, f"{metric}_critical_atls.png"))
            plt.close()

    # 3. Health Trends: Current Release Metrics trend for each metric
    for metric in metrics.get("health_trends", {}):
        versions = []
        cur_values = []
        for version, rows in metrics["health_trends"].items():
            for row in rows:
                value = row.get(f"Current Release Metrics (WST {version})", "")
                if value not in ("", "N/A", None):
                    try:
                        cur = float("".join([c for c in str(value) if c.isdigit() or c == "."]))
                        versions.append(version)
                        cur_values.append(cur)
                    except Exception:
                        continue
        if versions and cur_values:
            plt.figure()
            plt.plot(versions, cur_values, marker='o')
            plt.title(f"Current Release Metrics Trend - {metric}")
            plt.xlabel('Version')
            plt.ylabel('Current Release Metrics')
            plt.tight_layout()
            plt.savefig(os.path.join(viz_folder, f"{metric}_health_trend.png"))
            plt.close()
